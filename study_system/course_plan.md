...
所以后面你打开一课时，看到的不只是“知道学什么”，而应该是“可以开始学、开始记、开始判断”。

## 环境依赖原则

课程必须区分“现在能学”和“需要真实环境才能验证”的内容。
如果没有板子、相机、SDK、模型或 ROS2 环境，不应该硬讲成已经掌握。

环境依赖分级：

| 等级 | 含义 | 学习方式 |
| --- | --- | --- |
| A | 本地即可验证 | 必须有代码实验或命令观察 |
| B | 可用模拟替代 | 用本地 mock、日志样例、配置样例练方法 |
| C | 需要真实硬件/SDK | 只讲概念边界和检查清单，不声称已经掌握现场排障 |

后续带学时，如果课程属于 B 或 C，必须先说明限制，再决定是否继续。
不能把没有证据、没有环境、不能执行的内容讲成结论。
