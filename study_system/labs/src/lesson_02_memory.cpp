...
std::filesystem::path CreateTempFile(std::size_t bytes) {
  std::filesystem::path path =
      std::filesystem::temp_directory_path() / "study_system_memory_lab.bin";
...
int RunReadVsMmap(std::size_t size_mb, int rounds) {
  const std::size_t bytes = size_mb * 1024 * 1024;
  ...
  const auto read_sum = SequentialRead(fd, bytes, rounds);
  ...
  std::cout << "mode=read_vs_mmap size_mb=" << size_mb << " rounds=" << rounds << "\n";
  ...
  std::cout << "Observation: the difference depends on access pattern and when costs appear.\n";
  return 0;
}
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (mode == "read_vs_mmap") {
      ...
      return RunReadVsMmap(size_mb, rounds);
    }
